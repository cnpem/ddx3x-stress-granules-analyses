...
def get_treatment_from_image_filename(im_fn):
    if "ARS" in im_fn:
        return 'ARS'
    elif 'GLU_40min' in im_fn:
        return 'GLU-40min'
    elif 'GLU_20min' in im_fn:
        return 'GLU-20min'
    elif 'CT' in im_fn:
        return 'CT'
    else:
...
# plot
fig_example, axs = plt.subplots(8,3, figsize=(9,25))
example_number = 3
    ...
    for im_fn in glob(im_path):
        ...
        if image_number == example_number:
            ...
            microplot.microshow(
                ...
                scalebar_font_size=10, scalebar_thickness=0.01,
                rescale_type='limits',
                limits=[[min_nuclei, max_nuclei], [min_cyto, max_cyto]])           # fig_basename = re.search(r'\[(.*?)\.lif\]', im_fn).group(1)
...
<!-- 
```{python}
# cells_df['group'] = cells_df['group'].replace(
#     {'20240913_R326H_GFP_placa1_2908__CT_Z': 'CT',
#      '20240912_R326H_GFP_placa1_2908__ARS_Z': 'ARS',
#      '20240911_R326H_GFP_placa3_2908__GLU_40min_Z':'GLU 40min',
#      '20240912_R326H_GFP_placa2_2908__GLU_20min_Z': 'GLU 20min'})
# cells_df = cells_df.sort_values('group', key=lambda x: pd.Categorical(x, categories=['ARS', 'GLU 40min', 'GLU 20min', 'CT'], ordered=True))
cells_df
```

```{python}
# granules_df['group'] = granules_df['group'].replace(
#     {'20240913_R326H_GFP_placa1_2908__CT_Z': 'CT',
#      '20240912_R326H_GFP_placa1_2908__ARS_Z': 'ARS',
#      '20240911_R326H_GFP_placa3_2908__GLU_40min_Z':'GLU 40min',
#      '20240912_R326H_GFP_placa2_2908__GLU_20min_Z': 'GLU 20min'})
# granules_df = granules_df.sort_values('group', key=lambda x: pd.Categorical(x, categories=['ARS', 'GLU 40min', 'GLU 20min', 'CT'], ordered=True))
granules_df
```

```{python}
fig, axs = plt.subplots(1,2, figsize=(8,3), sharey=True,  constrained_layout=True)
...
sns.violinplot(data=granules_df[granules_df.group == 'WT'], 
               ...
               inner='box', 
               ax=axs[0], 
               order=['CT', 'ARS', 'GLU-20min', 'GLU-40min'], 
               color='k', 
               density_norm='width',
               linewidth=1)
